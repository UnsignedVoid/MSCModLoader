...
namespace MSCPatcher {
	public class MainData {
		...
		static byte[] data = { 0x41, 0x6d, 0x69, 0x73, 0x74, 0x65, 0x63, 0x68, 0x0d,
			0x00, 0x00, 0x00, 0x4d, 0x79, 0x20, 0x53, 0x75, 0x6d, 0x6d, 0x65, 0x72, 0x20,
			0x43, 0x61, 0x72 };
		static long offset = 0;
		static string mainDataPath = null;

		public static void LoadMainData(
				Label outputlog, Label resDialog, CheckBox resDialogCheck) {
			if (Form1.mscPath != "(unknown)") {
				...
				try {
					...
					MessageBox.Show(
							string.Format(
									"Failed to read data from file.{1}{1}Error details:{1}{0}",
		...
		public static void ApplyChanges(
				bool output_log, bool resDialog, bool mb = true) {
			if (mainDataPath != null) {
				try {
					using (var stream = new FileStream(
										 mainDataPath, FileMode.Open, FileAccess.ReadWrite)) {
						// output_log.txt
						stream.Position = offset + 115;

						if (output_log) stream.WriteByte(0x01);
						else
							stream.WriteByte(0x00);

						// resolution dialog
						stream.Position = offset + 96;
						if (!resDialog) stream.WriteByte(0x01);
						else
							stream.WriteByte(0x00);
					...
					MessageBox.Show(
							string.Format(
									"Failed to write data to file.{1}{1}Error details:{1}{0}",
									e.Message, Environment.NewLine),
							"Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
					Log.Write("Error", true, true);
					Log.Write(e.Message);
