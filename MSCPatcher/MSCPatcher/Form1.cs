...
namespace MSCPatcher {
	public partial class Form1 : Form {
		...
		private string mdPath = Path.Combine(
				...
				@"MySummerCar\Mods");
		private string adPath = Path.GetFullPath(Path.Combine(
		...
		private bool oldPatchFound =
				false; // 0.1 patch found, recover and patch Assembly-CSharp.original.dll and
							 // cleanup unused files
		private bool oldFilesFound = false; // 0.1 files found, but no patch, cleanup
																				// files and patch new Assembly-CSharp.dll
		private bool mscloaderUpdate =
				false; // new MSCLoader.dll found, but no new patch needed.
		...
		public Form1() {
			...
			InitializeComponent();
			Log.logBox = logBox;
			if (File.Exists("Assembly-CSharp.dll") || File.Exists("UnityEngine.dll") ||
					File.Exists("mysummercar.exe") || File.Exists("mainData") ||
					File.Exists("mono.dll") || File.Exists("CSteamworks.dll") ||
					Directory.Exists(
							"References")) // check if idiot unpacked this to game folder.
			{
				if (MessageBox.Show("Did you read the instructions? (Readme.txt)",
								"Question for you!", MessageBoxButtons.YesNo,
								MessageBoxIcon.Question) == DialogResult.Yes) {
					MessageBox.Show(
							string.Format(
									"Why are you lying?{0}Or maybe you can't read?{0}If you could read, you would know where not to unpack files.",
									Environment.NewLine),
							"You are a liar", MessageBoxButtons.OK, MessageBoxIcon.Error);
				} else {
					MessageBox.Show(
							string.Format(
									"Yes I see.{0}Go back to readme and you will know you would know where not to unpack files.",
									Environment.NewLine),
							"Read manual", MessageBoxButtons.OK, MessageBoxIcon.Warning);
				}
				Environment.Exit(0);
			}
			if (!File.Exists("MSCLoader.dll")) {
				MessageBox.Show(
						string.Format(
								"Don't run this file from zip folder. Unpack all files and start patcher again.",
								Environment.NewLine),
						"Read the freaking instructions", MessageBoxButtons.OK,
			...
			try {
				...
				if (mscLoaderVersion.FileBuildPart != 0)
					currentVersion =
							string.Format("{0}.{1}.{2}", mscLoaderVersion.FileMajorPart,
									mscLoaderVersion.FileMinorPart, mscLoaderVersion.FileBuildPart);
				else
					currentVersion = string.Format("{0}.{1}", mscLoaderVersion.FileMajorPart,
							mscLoaderVersion.FileMinorPart);
				...
				if (i != 0) {
					Log.Write(string.Format("{2}MCSLoader v{0}, New version available: v{1}",
							currentVersion, version.Trim(), Environment.NewLine));
					if (MessageBox.Show(
									string.Format(
											"New version is available: v{0}, wanna check it out?",
											version.Trim()),
									"MCSLoader v" + currentVersion, MessageBoxButtons.YesNo,
						...
						try {
							...
							MessageBox.Show(
									string.Format(
											"Failed to open update info!{1}{1}Error details:{1}{0}",
					...
					statusBarLabel.Text =
							string.Format("New version available: v{0}", version.Trim());
				} else if (i == 0) {
					Log.Write(
							string.Format("{1}MCSLoader v{0} is up to date, no new version found.",
				...
				Log.Write(string.Format(
						"Check for new version failed with error: {0}", e.Message));
				statusBarLabel.Text = "MSCPatcher Ready!";
				Log.Write(e.ToString());
			}
			...
			if (mscPath != "(unknown)") {
				...
				Log.Write(string.Format(
						"Game folder set to: {0}{1}", mscPath, Environment.NewLine));
		...
		public void PatchStarter() {
			if (modPath == Path.GetFullPath(".")) {
				throw new Exception(
						"Modloader Files unpacked into mod folder. Cannot install.");
			}
			if (oldFilesFound) {
				Log.Write("Cleaning old files!", true, true);
				// Remove old 0.1 unused files and patch game
				Patcher.DeleteIfExists(Path.Combine(
						mscPath, @"mysummercar_Data\Managed\Assembly-CSharp.original.dll"));
				Patcher.DeleteIfExists(
						Path.Combine(mscPath, @"mysummercar_Data\Managed\Mono.Cecil.dll"));
				Patcher.DeleteIfExists(
						Path.Combine(mscPath, @"mysummercar_Data\Managed\Mono.Cecil.Rocks.dll"));
				Patcher.DeleteIfExists(
						Path.Combine(mscPath, @"mysummercar_Data\Managed\MSCLoader.dll"));
				Patcher.DeleteIfExists(
						Path.Combine(mscPath, @"mysummercar_Data\Managed\MSCPatcher.exe"));
				Patcher.DeleteIfExists(
						Path.Combine(mscPath, @"mysummercar_Data\Managed\System.Xml.dll"));
				...
				if (File.Exists(string.Format("{0}.backup", AssemblyFullPath))) {
					...
					MessageBox.Show(
							string.Format(
									"Backup file not found in:{1}{0}{1}Can't continue{1}{1}Please check integrity files in steam, to recover original file.",
									String.Format("{0}.backup", AssemblyFullPath),
									Environment.NewLine),
							"Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
				}
				StartPatching();
				...
				if (File.Exists(Path.Combine(mscPath,
								@"mysummercar_Data\Managed\Assembly-CSharp.original.dll"))) {
					if (File.Exists(AssemblyFullPath)) {
						Log.Write("Recovering backup file!", true, true);

						Patcher.DeleteIfExists(AssemblyFullPath);

						File.Move(Path.Combine(mscPath,
													@"mysummercar_Data\Managed\Assembly-CSharp.original.dll"),
								AssemblyFullPath);
					...
					Patcher.DeleteIfExists(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\Mono.Cecil.dll"));
					Patcher.DeleteIfExists(Path.Combine(
							mscPath, @"mysummercar_Data\Managed\Mono.Cecil.Rocks.dll"));
					Patcher.DeleteIfExists(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\MSCLoader.dll"));
					Patcher.DeleteIfExists(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\MSCPatcher.exe"));
				...
				using (TextWriter tw =
									 File.CreateText(Path.Combine(mscPath, @"doorstop_config.ini"))) {
					...
					switch (InitMethod) {
					case "Init_MD": tw.WriteLine(@"mods=MD"); break;
					case "Init_GF": tw.WriteLine(@"mods=GF"); break;
					case "Init_AD": tw.WriteLine(@"mods=AD"); break;
					default: tw.WriteLine(@"mods=GF"); break;
					}
				...
				Log.Write("");
				MessageBox.Show("Update successfull!", "Success", MessageBoxButtons.OK,
			...
			try {
				...
				if (is64bin) {
					...
					if (File.Exists(Path.GetFullPath(Path.Combine("w64.dll", "")))) {
						File.Copy(Path.GetFullPath(Path.Combine("w64.dll", "")),
								Path.Combine(mscPath, @"winhttp.dll"));
						Log.Write("Copying new file.....winhttp.dll");
					} else {
						throw new FileNotFoundException(
								"File \"w64.dll\" not found, please redownload modlaoder and/or unpack all files",
								"w64.dll");
					}
				} else {
					Patcher.DeleteIfExists(Path.Combine(mscPath, @"winhttp.dll"));
					if (File.Exists(Path.GetFullPath(Path.Combine("w32.dll", "")))) {
						File.Copy(Path.GetFullPath(Path.Combine("w32.dll", "")),
								Path.Combine(mscPath, @"winhttp.dll"));
						Log.Write("Copying new file.....winhttp.dll");
					} else {
						throw new FileNotFoundException(
								"File \"w32.dll\" not found, please redownload modlaoder and/or unpack all files",
								"w32.dll");
					}
				}
				Patcher.CopyCoreAssets(modPath);
				...
				Log.Write("");
				MessageBox.Show("Install successfull!", "Success", MessageBoxButtons.OK,
						MessageBoxIcon.Information);
				statusBarLabel.Text = "Install successfull!";
			} catch (Exception ex) {
				MessageBox.Show(string.Format("Error while installing: {0}", ex.Message),
						"Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				Log.Write("Error", true, true);
				Log.Write(ex.Message);
				Log.Write(ex.ToString());
		...
		private void button1_Click(object sender, EventArgs e) {
			...
			if (AssemblyFullPath != null) {
				if (MDradio.Checked) {
					...
					if (Directory
									.GetParent(Environment.GetFolderPath(
											Environment.SpecialFolder.MyDocuments))
									.ToString() == "OneDrive")
						if (MessageBox.Show(
										$"You are about to set your mods folder to a directory that is part of OneDrive cloud.{Environment.NewLine}This is not supported due to On-Demand files that may cause issues.{Environment.NewLine}{Environment.NewLine}Do you want to continue?",
										"Cloud folder warning", MessageBoxButtons.YesNo,
					...
					if (isCloudIgnore) {
						try {
							...
							MessageBox.Show(
									string.Format("Error:{1}{0}", ex.Message, Environment.NewLine),
									"Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
							Log.Write("Error", true, true);
							Log.Write(ex.Message);
							Log.Write(ex.ToString());
				...
				MessageBox.Show("Select game path first", "Error path unknown",
		...
		private void button2_Click(object sender, EventArgs e) {
			if (selectPathToMSC.ShowDialog() == DialogResult.OK) {
				...
				Log.Write(string.Format(
						"Game folder is saved as: {0}{1}", mscPath, Environment.NewLine));
				MainData.LoadMainData(OutputlogLabel, resDialogLabel, resDialogCheck);
				CheckPatchStatus();
				DebugStatusInfo();
				// Check64Info();
		...
		void CheckPatchStatus() {
			...
			try {
				...
				if (MDradio.Checked)
					isInjected = IsPatched(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\Assembly-CSharp.dll"),
							"PlayMakerArrayListProxy", "Awake", "MSCLoader.dll",
							"MSCLoader.ModLoader", "Init_MD");
				else if (GFradio.Checked)
					isInjected = IsPatched(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\Assembly-CSharp.dll"),
							"PlayMakerArrayListProxy", "Awake", "MSCLoader.dll",
							"MSCLoader.ModLoader", "Init_GF");
				else if (ADradio.Checked)
					isInjected = IsPatched(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\Assembly-CSharp.dll"),
							"PlayMakerArrayListProxy", "Awake", "MSCLoader.dll",
							"MSCLoader.ModLoader", "Init_AD");

				if (isInjected) newpatchfound = true;
				else
					newpatchfound = false;
			} catch (Exception ex) {
				MessageBox.Show(string.Format("Patch checking error: {0}", ex.Message),
						"Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
				Log.Write("Error", true, true);
				Log.Write(ex.Message);
				Log.Write(ex.ToString());
			...
			if (!newpatchfound) {
				try {
					bool isInjected = IsPatched(
							Path.Combine(mscPath, @"mysummercar_Data\Managed\Assembly-CSharp.dll"),
							"StartGame", ".ctor", "MSCLoader.dll", "MSCLoader.ModLoader", "Init");
					if (isInjected) oldpatchfound = true;
					else
						oldpatchfound = false;
				} catch (Exception ex) {
					MessageBox.Show(string.Format("Patch checking error: {0}", ex.Message),
			...
			if (!newpatchfound && !oldpatchfound) {
				if (File.Exists(
								Path.Combine(mscPath, @"mysummercar_Data\Managed\MSCLoader.dll")) &&
						File.Exists(Path.Combine(mscPath, @"winhttp.dll")) &&
						File.Exists(Path.Combine(mscPath, @"doorstop_config.ini"))) {
					proxypatchfound = true;
				} else if (File.Exists(Path.Combine(mscPath,
											 @"mysummercar_Data\Managed\Assembly-CSharp.original.dll"))) {
					statusLabelText.Text =
							"Not installed, but MSCLoader 0.1 found (upgrade required)";
					Log.Write(
							"Proxy patch not found, but MSCLoader 0.1 files found (upgrade required)");
					button1.Text = "Install MSCLoader";
					button1.Enabled = true;
					oldFilesFound = true;
				} else if (File.Exists(Path.Combine(
											 mscPath, @"mysummercar_Data\Managed\MSCLoader.dll")) &&
						File.Exists(Path.Combine(
								mscPath, @"mysummercar_Data\Managed\Assembly-CSharp.dll.backup"))) {
					statusLabelText.Text =
							"Not installed, but MSCLoader files found (upgrade required)";
					Log.Write(
							"Proxy patch not found, but MSCLoader files found (upgrade required)");
					button1.Text = "Install MSCLoader";
					button1.Enabled = true;
					isgameUpdated = true;
				} else {
			...
			if (proxypatchfound) {
				if (MD5HashFile(
					...
					statusLabelText.Text =
							"Installed, but MSCLoader update available, Update?";
		...
		private void button3_Click(object sender, EventArgs e) {
			if (MessageBox.Show("Do you want to remove MSCLoader from game?", "Remove?",
				...
				try {
					...
					Log.Write("");
					MessageBox.Show("MSCLoader removed successfully!", "Success!",
		...
		void LaunchMSCsruSteam() {
			try {
				...
				MessageBox.Show(
						string.Format(
								"Failed to run MSC, is steam installed correctly?{1}{1}Error details:{1}{0}",
								ex.Message, Environment.NewLine),
						"Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
				Log.Write("Error", true, true);
				Log.Write(ex.Message);
		...
		private void button4_Click(object sender, EventArgs e) { LaunchMSCsruSteam(); }
		...
		private void enDebug_Click(object sender, EventArgs e) {
			if (debugCheckbox.Checked) {
				...
				MessageBox.Show("Please select that you understand what debugging is.",
						"Read info first", MessageBoxButtons.OK, MessageBoxIcon.Warning);
			}
		}
		...
		private void linkDebug_LinkClicked(
				object sender, LinkLabelLinkClickedEventArgs e) {
			try {
				Process.Start(
						"https://github.com/piotrulos/MSCModLoader/wiki/Debugging-your-mods-in-Visual-Studio");
			} catch (Exception ex) {
				MessageBox.Show(
						string.Format("Failed to open url!{1}{1}Error details:{1}{0}",
								ex.Message, Environment.NewLine),
						"Error!", MessageBoxButtons.OK, MessageBoxIcon.Error);
				Log.Write("Error", true, true);
				Log.Write(ex.Message);
		...
		private void PatchThis(string mainPath, string assemblyToPatch,
				string assemblyType, string assemblyMethod, string loaderAssembly,
				string loaderType, string loaderMethod) {
			DefaultAssemblyResolver resolver = new DefaultAssemblyResolver();
			resolver.AddSearchDirectory(mainPath);

			ModuleDefinition assembly = ModuleDefinition.ReadModule(
					mainPath + "/" + assemblyToPatch,
					new ReaderParameters { ReadWrite = true, AssemblyResolver = resolver });
			ModuleDefinition loader =
					ModuleDefinition.ReadModule(mainPath + "/" + loaderAssembly);
			MethodDefinition methodToInject =
					loader.GetType(loaderType).Methods.Single(x => x.Name == loaderMethod);
			MethodDefinition methodToHook =
					assembly.GetType(assemblyType)
							.Methods.First(x => x.Name == assemblyMethod);

			Instruction loaderInit =
					Instruction.Create(OpCodes.Call, assembly.ImportReference(methodToInject));
		...
		private bool IsPatched(string assemblyToPatch, string assemblyType,
				string assemblyMethod, string loaderAssembly, string loaderType,
				string loaderMethod) {
			...
			foreach (Instruction instruction in methodToHook.Body.Instructions) {
				if (instruction.OpCode.Equals(OpCodes.Call) &&
						instruction.Operand.ToString().Equals(
								$"System.Void {loaderType}::{loaderMethod}()")) {
					assembly.Dispose();
					loader.Dispose();
					return true;
				}
