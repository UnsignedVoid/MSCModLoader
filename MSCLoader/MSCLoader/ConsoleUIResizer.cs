...
#pragma warning disable CS1591
	...
	public class ConsoleUIResizer : MonoBehaviour, IDragHandler {
		...
		public void OnDrag(PointerEventData eventData) {
			ClampToBorder();
			if (Xresizer) {
				if (clampedToRight && eventData.delta.x > 0) return;
				m_transform.position += new Vector3(eventData.delta.x, 0f);
				if (m_transform.anchoredPosition.x <= 0)
					m_transform.anchoredPosition =
							new Vector2(0f, m_transform.anchoredPosition.y);
				m_scrollbar.anchoredPosition = new Vector2(
						m_transform.anchoredPosition.x, m_scrollbar.anchoredPosition.y);
				m_submitBtn.anchoredPosition =
						new Vector2(m_transform.anchoredPosition.x, 0f);
				m_logview.sizeDelta = new Vector2(336f + m_transform.anchoredPosition.x,
						120f + m_logview.anchoredPosition.y);
				m_inputField.sizeDelta =
						new Vector2(326f + m_transform.anchoredPosition.x, 30f);
				m_otherResizer.sizeDelta =
						new Vector2(346f + m_transform.anchoredPosition.x, -8f);
			} else {
				if (clampedToTop && eventData.delta.y > 0) return;
				m_transform.position += new Vector3(0f, eventData.delta.y);
				if (m_transform.anchoredPosition.y <= -60f)
					m_transform.anchoredPosition =
							new Vector2(m_transform.anchoredPosition.x, -60f);
				m_logview.anchoredPosition = new Vector2(0f, m_transform.anchoredPosition.y);
				m_scrollbar.anchoredPosition = new Vector2(
						m_scrollbar.anchoredPosition.x, m_transform.anchoredPosition.y);
				m_otherResizer.anchoredPosition = new Vector2(
						m_otherResizer.anchoredPosition.x, m_transform.anchoredPosition.y);
				m_logview.sizeDelta = new Vector2(336f + m_otherResizer.anchoredPosition.x,
						120f + m_logview.anchoredPosition.y);
				m_scrollbar.sizeDelta =
						new Vector2(10f, 120f + m_scrollbar.anchoredPosition.y);
				m_otherResizer.sizeDelta =
						new Vector2(-8f, 150f + m_otherResizer.anchoredPosition.y);
			}
		}
		...
		public void OnDisable() { OnMouseExit(); }
