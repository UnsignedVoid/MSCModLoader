...
#pragma warning disable CS1591
	public class OBJLoader {
		...
		public Mesh ImportFile(string filePath) {
			...
			int i = 0;
			/* The following foreach loops through the facedata and assigns the appropriate
			 * vertex, uv, or normal for the appropriate Unity mesh array.
			 */
			...
			using (StringReader reader = new StringReader(entireText)) {
				...
				while (currentText != null) {
					if (!currentText.StartsWith("f ") && !currentText.StartsWith("v ") &&
						...
						brokenString = currentText.Split(
								splitIdentifier, 50); // Split the line into an array, separating the
																			// original line by blank spaces
						...
						case "f":
							...
							triangles = triangles +
									3 *
											(brokenString.Length -
													2); /*brokenString.Length is 3 or greater since a face must
														have at least 3 vertices.  For each additional vertice,
														there is an additional triangle in the mesh (hence this
														formula).*/
							break;
						}
						currentText = reader.ReadLine();
		...
		private static void populateMeshStruct(ref meshStruct mesh) {
			...
			using (StringReader reader = new StringReader(entireText)) {
				...
				while (currentText != null) {
					if (!currentText.StartsWith("f ") && !currentText.StartsWith("v ") &&
							!currentText.StartsWith("vt ") && !currentText.StartsWith("vn ") &&
							!currentText.StartsWith("g ") && !currentText.StartsWith("usemtl ") &&
							!currentText.StartsWith("mtllib ") &&
							!currentText.StartsWith("vt1 ") && !currentText.StartsWith("vt2 ") &&
						...
						case "f":
							...
							while (j < brokenString.Length && ("" + brokenString[j]).Length > 0) {
								...
								brokenBrokenString = brokenString[j].Split(
										splitIdentifier2, 3); // Separate the face into individual
																					// components (vert, uv, normal)
								temp.x = Convert.ToInt32(brokenBrokenString[0]);
								if (brokenBrokenString.Length >
										1) // Some .obj files skip UV and normal
								{
							...
							while (j + 2 <
									brokenString
											.Length) // Create triangles out of the face data.  There will
															 // generally be more than 1 triangle per face.
							{
