	...
	public class ConsoleController {
		...
		internal class CommandRegistration {
			...
			public CommandRegistration(
					string command, CommandHandler handler, string help, bool showInHelp) {
		...
		// Note that strings submitted to appendLogLine with embedded newlines will be
		// counted as a single line.
#if DevMode
		...
		internal Dictionary<string, CommandRegistration> commands =
				new Dictionary<string, CommandRegistration>();

		public string[] log { get; private set; } // Copy of scrollback as an array for
																							// easier use by ConsoleView
		...
		public void RegisterCommand(
				string command, CommandHandler handler, string help, bool inHelp = true) {
		...
		public void RegisterCommand(string command, CommandHandler handler, string help,
				string alias, bool inHelp = true) {
			CommandRegistration cmd =
					new CommandRegistration(command, handler, help, inHelp);
		...
		public void RunCommandString(string commandString) {
			if (!string.IsNullOrEmpty(commandString)) {
				AppendLogLine(string.Format("{1}<b><color=orange>></color></b> {0}",
						commandString, Environment.NewLine));

				string[] commandSplit = ParseArguments(commandString);
				string[] args = new string[0];
				if (commandSplit.Length < 1) {
					AppendLogLine(string.Format(
							"<color=red>Unable to process command:</color> <b>{0}</b>",
							commandString));
					return;
		...
		void RunCommand(string command, string[] args) {
			if (!string.IsNullOrEmpty(command)) {
				if (!commands.TryGetValue(command, out CommandRegistration reg)) {
					AppendLogLine(string.Format(
							"Unknown command <b><color=red>{0}</color></b>, type <color=lime><b>help</b></color> for list.",
							command));
				} else {
					if (reg.handler == null) {
						AppendLogLine(string.Format(
		...
		static string[] ParseArguments(string commandString) {
			...
			return new string(parmCharsArr)
					.Split(new char[] { '\n' }, StringSplitOptions.RemoveEmptyEntries);
		}

		void HelpCommand(string[] args) {
			ModConsole.Print("<color=green><b>Available commands:</b></color>");
			CommandRegistration[] cmds = commands.Values.GroupBy(x => x.command)
																			 .Select(g => g.First())
																			 .Distinct()
																			 .ToArray();
			for (int i = 0; i < cmds.Length; i++) {
				if (cmds[i].showInHelp)
					AppendLogLine(string.Format("<color=orange><b>{0}</b></color>: {1}",
							cmds[i].command, cmds[i].help));
			}
			if (ModLoader.GetCurrentScene() != CurrentScene.Game) {
				AppendLogLine(
						"<b><color=red>More commands may appear after you load a save...</color></b>");
			}
		}
	}
