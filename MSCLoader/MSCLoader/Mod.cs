namespace MSCLoader {
	...
	public abstract class Mod {
		...
		internal virtual int modErrors {
			get => errorCount;
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or
		...
		internal virtual string compiledVersion {
			get => compiledVer;
			set => compiledVer = value;
		}
		...
		internal virtual ModsManifest metadata {
			get => modMetadata;
			set => modMetadata = value;
		}
		internal virtual ModsManifest RemMetadata {
			get => rmodMetadata;
			set => rmodMetadata = value;
		}
		...
		public virtual void OnMenuLoad() {
			if (LoadInMenu)
				ModConsole.Error(string.Format(
						"<b>LoadInMenu</b> is set to <b>true</b> for mod: <b>{0}</b> but <b>OnMenuLoad()</b> is empty.",
						ID));
		}
		...
		/// <summary>
		/// Called once, after ALL mods has finished OnLoad() and when SecondPass is set
		/// to true (Executed still before first pass of Update(), but NOT exectued if
		/// OnLoad() failed with error)
		/// </summary>
		...
		/// </summary>
		/// <example>See:
		/// https://docs.unity3d.com/500/Documentation/Manual/GUIScriptingGuide.html
		/// </example>
