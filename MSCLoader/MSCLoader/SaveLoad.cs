...
#pragma warning restore CS1591
	...
	public class SaveLoad {
		...
		public static void SaveGameObject(Mod mod, GameObject g, string fileName) {
			...
			SaveDataList s = new SaveDataList { name = g.name, pos = g.transform.position,
		...
		public static void LoadGameObject(Mod mod, string fileName) {
			...
			go.transform.rotation =
					Quaternion.Euler(data.save[0].rotX, data.save[0].rotY, data.save[0].rotZ);
		}
		...
		public static void SerializeSaveFile<T>(
				Mod mod, T saveDataClass, string fileName) {
		...
		public static T DeserializeSaveFile<T>(Mod mod, string fileName)
				where T : new() {
			string path = Path.Combine(ModLoader.GetModSettingsFolder(mod), fileName);
			if (File.Exists(path)) {
