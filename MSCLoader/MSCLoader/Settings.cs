...
#pragma warning restore CS1591
	...
	public class Settings {
		...
		/// </summary>
		public static void HideResetAllButton(Mod mod) => modSettingsDefault.Add(
				new Settings("MSCL_HideResetAllButton", null, null) { Mod = mod });
		/// <summary>
		...
		public static List<Settings> Get(Mod mod) => modSettings.FindAll(
				x => x.Mod == mod);
		...
		/// <param name="mod">The mod to get the settings for.</param>
		/// <returns>List of Settings for the mod.</returns>
		public static List<Settings> GetDefault(Mod mod) => modSettingsDefault.FindAll(
				x => x.Mod == mod);
		...
		public static void AddCheckBox(Mod mod, Settings setting) {
			setting.Mod = mod;
			modSettingsDefault.Add(
					new Settings(setting.ID, setting.Name, setting.Value) { Mod = mod });
		...
		/// <summary>
		/// Add checkbox to settings menu with group, only one checkbox can be set to
		/// <see langword="true"/> in this group
		/// </summary>
		/// <param name="mod">Your mod instance</param>
		/// <param name="setting">Your settings variable</param>
		/// <param name="group">Unique group name, same for all checkboxes that will be
		/// grouped</param> <example><code source="SettingsExamples.cs"
		/// region="AddCheckBoxGroup" lang="C#" title="Add grouped checkboxes"
		/// /></example>
		...
		public static void AddResetButton(Mod mod, string name, Settings[] sets) {
			if (sets != null) {
				...
				ModConsole.Error(
						$"[<b>{mod.ID}</b>] AddResetButton: provide at least one setting to reset.");
			}
		}
		/// <summary>
		...
		public static void AddButton(
				Mod mod, Settings setting, string description = null) => AddButton(mod,
				setting, new UnityEngine.Color32(0, 113, 166, 255),
				new UnityEngine.Color32(0, 153, 166, 255),
				new UnityEngine.Color32(0, 183, 166, 255), description);
		/// <summary>
		/// Add button that can execute function.
		/// </summary>
		/// <param name="mod">Your mod instance</param>
		...
		/// <example><code source="SettingsExamples.cs" region="AddButton" lang="C#"
		/// title="Add button" /></example>
		public static void AddButton(Mod mod, Settings setting,
				UnityEngine.Color normalColor, UnityEngine.Color highlightedColor,
				UnityEngine.Color pressedColor, string description = null) => AddButton(mod,
				setting, normalColor, highlightedColor, pressedColor,
				UnityEngine.Color.white, description);
		/// <summary>
		/// Add button that can execute function.
		/// </summary>
		/// <param name="mod">Your mod instance</param>
		...
		public static void AddButton(Mod mod, Settings setting,
				UnityEngine.Color normalColor, UnityEngine.Color highlightedColor,
				UnityEngine.Color pressedColor, UnityEngine.Color buttonTextColor,
		...
		public static void AddSlider(Mod mod, Settings setting, int minValue,
				int maxValue) => AddSlider(mod, setting, minValue, maxValue, null);
		...
		/// <param name="maxValue">Max value of slider</param>
		/// <param name="minValue">Min value of slider</param>
		/// <param name="textValues">Array of text values (array index equals to slider
		/// value)</param> <example><code source="SettingsExamples.cs" region="AddSlider"
		/// lang="C#" title="Add Slider" /></example>
		public static void AddSlider(
				Mod mod, Settings setting, int minValue, int maxValue, string[] textValues) {
			setting.Mod = mod;
			modSettingsDefault.Add(
					new Settings(setting.ID, setting.Name, setting.Value) { Mod = mod });
			...
			if (setting.Value.GetType() != typeof(float) ||
					setting.Value.GetType() != typeof(string)) {
				...
				if (textValues == null) {
					...
					if (textValues.Length <= (maxValue - minValue)) {
						ModConsole.Error(
								$"[<b>{mod.ID}</b>] AddSlider: array of textValues is smaller than slider range (min to max).");
					}
				}
		...
		/// <example><code source="SettingsExamples.cs" region="AddSlider" lang="C#"
		/// title="Add Slider" /></example>
		public static void AddSlider(Mod mod, Settings setting, float minValue,
				float maxValue) => AddSlider(mod, setting, minValue, maxValue, 2);
		...
		/// <param name="maxValue">Max value of slider</param>
		/// <param name="minValue">Min value of slider</param>
		/// <param name="decimalPoints">Round value to set number of decimal points
		/// (default = 2)</param> <example><code source="SettingsExamples.cs"
		/// region="AddSlider" lang="C#" title="Add Slider" /></example>
		public static void AddSlider(Mod mod, Settings setting, float minValue,
				float maxValue, int decimalPoints) {
			setting.Mod = mod;
			modSettingsDefault.Add(
					new Settings(setting.ID, setting.Name, setting.Value) { Mod = mod });
			setting.Vals = new object[5];
			...
			if (decimalPoints < 0) {
				ModConsole.Error(
						$"[<b>{mod.ID}</b>] AddSlider: decimalPoints cannot be negative.");
			}
		}
		...
		public static void AddTextBox(
				Mod mod, Settings setting, string placeholderText) => AddTextBox(mod,
				setting, placeholderText, UnityEngine.Color.white);
		...
		public static void AddTextBox(Mod mod, Settings setting, string placeholderText,
				UnityEngine.Color titleTextColor) => AddTextBox(mod, setting,
				placeholderText, titleTextColor, InputField.ContentType.Standard);
		...
		public static void AddTextBox(Mod mod, Settings setting, string placeholderText,
				UnityEngine.Color titleTextColor, InputField.ContentType contentType) {
			setting.Mod = mod;
			modSettingsDefault.Add(
					new Settings(setting.ID, setting.Name, setting.Value) { Mod = mod });
		...
		public static void AddHeader(Mod mod, string HeaderTitle) => AddHeader(
				mod, HeaderTitle, UnityEngine.Color.blue, UnityEngine.Color.white);
		...
		public static void AddHeader(Mod mod, string HeaderTitle,
				UnityEngine.Color backgroundColor) => AddHeader(mod, HeaderTitle,
		...
		public static void AddHeader(Mod mod, string HeaderTitle,
				UnityEngine.Color backgroundColor, UnityEngine.Color textColor) {
