...
#pragma warning disable CS1591
	public class CommandLogAll : ConsoleCommand {
		...
		public override string Help =>
				"Log <b>ALL</b> errors/warnings (Warning! May spam console)";
		...
		public override void Run(string[] args) {
			if (args.Length > 0) {
				...
				if (args[0].ToLower() == "help") {
					ModConsole.Print(
							$"<color=green><b>Available settings:</b></color>{Environment.NewLine}" +
					...
					if (args.Length == 2) {
						if (args[1].ToLower() == "true") ModLoader.LogAllErrors = true;
						else
							ModLoader.LogAllErrors = false;
						ModConsole.Print(string.Format(
								"<color=orange>Log All errors for mods set to <b>{0}</b></color>",
								ModLoader.LogAllErrors));
					} else {
					...
					if (args.Length == 2) {
						if (args[1].ToLower() == "true") errors = true;
						else
							errors = false;
						ModConsole.Print(string.Format(
								"<color=orange>Log All errors set to <b>{0}</b></color>", errors));
					} else {
						errors = !errors;
						ModConsole.Print(string.Format(
								"<color=orange>Log All errors set to <b>{0}</b></color>", errors));
					...
					if (args.Length == 2) {
						if (args[1].ToLower() == "true") warnings = true;
						else
							warnings = false;
						ModConsole.Print(string.Format(
								"<color=orange>Log All warnings set to <b>{0}</b></color>",
								warnings));
					} else {
						warnings = !warnings;
						ModConsole.Print(string.Format(
					...
					if (args.Length == 2) {
						if (args[1].ToLower() == "true") messages = true;
						else
							messages = false;
						ModConsole.Print(string.Format(
								"<color=orange>Log All messages set to <b>{0}</b></color>",
								messages));
					} else {
						messages = !messages;
						ModConsole.Print(string.Format(
					...
					if (args.Length == 2) {
						if (args[1].ToLower() == "true") {
							...
							ModConsole.Print(string.Format(
									"<color=orange>Log everything set to <b>true</b></color>"));
							...
							ModLoader.LogAllErrors = false;
							ModConsole.Print(string.Format(
									"<color=orange>Log everything set to <b>false</b></color>"));
						}
					} else {
					...
					ModConsole.Warning(
							"<b>Usage:</b> log-all <mods|errors|warnings|messages|everything> [true|false]");
					ModConsole.Print(
							"Use <color=orange><b>log-all help</b></color> for more info");
				}
		...
		void HandleLog(string logString, string stackTrace, UnityEngine.LogType type) {
			...
			case UnityEngine.LogType.Error:
				if (errors)
					ModConsole.console.controller.AppendLogLine(
							$"<color=red><b>Error: </b>{logString}</color>");
			...
			case UnityEngine.LogType.Assert:
				if (messages)
					ModConsole.console.controller.AppendLogLine(
							$"<color=aqua>{logString}</color>");
				break;
			case UnityEngine.LogType.Warning:
				if (warnings)
					ModConsole.console.controller.AppendLogLine(
							$"<color=yellow><b>Warning: </b>{logString}</color>");
			...
			case UnityEngine.LogType.Exception:
				if (errors)
					ModConsole.console.controller.AppendLogLine(
							$"<color=red><b>Exception: </b>{logString}</color>");
				if ((bool)ModSettings_menu.dm_logST.GetValue())
					ModConsole.console.controller.AppendLogLine(
							$"<color=red>{stackTrace}</color>");
				if ((bool)ModSettings_menu.dm_operr.GetValue())
