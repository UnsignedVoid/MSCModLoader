...
#pragma warning disable CS1591
	public class SettingsView : MonoBehaviour {
		...
		public void SettingsList(Settings setting) {
			...
			case SettingsType.CheckBox:
				...
				checkbox.GetComponent<Toggle>().onValueChanged.AddListener(delegate {
					...
					if (setting.DoAction != null) setting.DoAction.Invoke();
				});
				checkbox.transform.SetParent(modSettingsList.transform, false);
				break;
			case SettingsType.CheckBoxGroup:
				GameObject group;
				if (modSettingsList.transform.FindChild(setting.Vals[0].ToString()) ==
						null) {
					...
					group = modSettingsList.transform.FindChild(setting.Vals[0].ToString())
											.gameObject;
				...
				if ((bool)setting.Value)
					checkboxG.GetComponent<Toggle>().group.NotifyToggleOn(
							checkboxG.GetComponent<Toggle>());
			...
			case SettingsType.Button:
				...
				setting.NameText =
						btn.transform.GetChild(0).GetChild(0).GetComponent<Text>();
				setting.NameText.text = setting.Name;
				btn.transform.GetChild(1).GetComponent<Text>().text =
						setting.Vals[0].ToString();
				btn.transform.GetChild(1).GetComponent<Text>().color =
						(Color)setting.Vals[4];
				if (setting.Vals[0].ToString() == null ||
						setting.Vals[0].ToString() == string.Empty)
					btn.transform.GetChild(1).gameObject.SetActive(false);
				btn.transform.GetChild(0).GetComponent<Button>().onClick.AddListener(
			...
			case SettingsType.RButton:
				...
				setting.NameText =
						rbtn.transform.GetChild(0).GetChild(0).GetComponent<Text>();
				setting.NameText.text = setting.Name;
				rbtn.transform.GetChild(0).GetChild(0).GetComponent<Text>().color =
						Color.black;
				rbtn.transform.GetChild(1).gameObject.SetActive(false);
				rbtn.transform.GetChild(0).GetComponent<Button>().onClick.AddListener(
						delegate {
							ModSettings_menu.ResetSpecificSettings(
									setting.Mod, (Settings[])setting.Vals[0]);
			...
			case SettingsType.Slider:
				...
				slidr.transform.GetChild(1).GetComponent<Text>().text =
						setting.Value.ToString();
				slidr.transform.GetChild(0).GetComponent<Slider>().minValue =
						float.Parse(setting.Vals[0].ToString());
				slidr.transform.GetChild(0).GetComponent<Slider>().maxValue =
						float.Parse(setting.Vals[1].ToString());
				slidr.transform.GetChild(0).GetComponent<Slider>().value =
						float.Parse(setting.Value.ToString());
				slidr.transform.GetChild(0).GetComponent<Slider>().wholeNumbers =
						(bool)setting.Vals[2];
				if (setting.Vals[3] != null) {
					slidr.transform.GetChild(1).GetComponent<Text>().text =
							((string[])setting.Vals[3])[int.Parse(setting.Value.ToString())];
				}
				slidr.transform.GetChild(0)
						.GetComponent<Slider>()
						.onValueChanged.AddListener(delegate {
							if ((bool)setting.Vals[2])
								...
								setting.Value = Math.Round(
										slidr.transform.GetChild(0).GetComponent<Slider>().value,
										int.Parse(setting.Vals[4].ToString()));
							if (setting.Vals[3] == null)
								slidr.transform.GetChild(1).GetComponent<Text>().text =
										setting.Value.ToString();
							else {
			...
			case SettingsType.TextBox:
				...
				txt.transform.GetChild(0).GetComponent<Text>().text =
						setting.Vals[0].ToString();
				txt.GetComponent<InputField>().contentType =
						(InputField.ContentType)setting.Vals[2];
				txt.GetComponent<InputField>().text = setting.Value.ToString();
				txt.GetComponent<InputField>().onValueChange.AddListener(
						delegate { setting.Value = txt.GetComponent<InputField>().text; });
			...
			case SettingsType.Header:
				...
				hdr.transform.GetChild(0).GetComponent<Text>().color =
						(Color)setting.Vals[2];
		...
		public void KeyBindsList(Keybind key) {
			...
			keyBind.AddComponent<KeyBinding>().modifierButton =
					keyBind.transform.GetChild(1).gameObject;
			keyBind.GetComponent<KeyBinding>().modifierDisplay =
					keyBind.transform.GetChild(1).GetChild(0).GetComponent<Text>();
			keyBind.GetComponent<KeyBinding>().keyButton =
					keyBind.transform.GetChild(3).gameObject;
			keyBind.GetComponent<KeyBinding>().keyDisplay =
					keyBind.transform.GetChild(3).GetChild(0).GetComponent<Text>();
			...
			if (selected_mod.isDisabled != ischecked) {
				...
				if (ischecked) {
					...
					ModConsole.Print(string.Format(
							"Mod <b><color=orange>{0}</color></b> is <color=red><b>Disabled</b></color>",
							selected_mod.Name));
				} else {
					selected_mod.OnModEnabled();
					ModConsole.Print(string.Format(
							"Mod <b><color=orange>{0}</color></b> is <color=green><b>Enabled</b></color>",
							selected_mod.Name));
				}
				ModSettings_menu.SaveSettings(selected_mod);
			}
		...
		public void ModDetailsShow(Mod mod) {
			...
			InfoTxt.text = string.Format(
					"<color=yellow>ID:</color> <b><color=lime>{0}</color></b>{1}", mod.ID,
					Environment.NewLine);
			InfoTxt.text += string.Format(
					"<color=yellow>Name:</color> <b><color=lime>{0}</color></b>{1}", mod.Name,
					Environment.NewLine);

			if (mod.hasUpdate)
				InfoTxt.text += string.Format(
						"<color=yellow>Version:</color> <b><color=orange>{0}</color></b> (<color=lime>{3} available</color>){2}(designed for <b><color=lime>v{1}</color></b>){2}",
						mod.Version, mod.compiledVersion, Environment.NewLine,
						mod.RemMetadata.version);
			else
				InfoTxt.text += string.Format(
						"<color=yellow>Version:</color> <b><color=lime>{0}</color></b>{2}(designed for <b><color=lime>v{1}</color></b>){2}",
						mod.Version, mod.compiledVersion, Environment.NewLine);

			InfoTxt.text += string.Format(
					"<color=yellow>Author:</color> <b><color=lime>{0}</color></b>",
					mod.Author);
			if (Application.loadedLevelName == "MainMenu") DisableMod.interactable = true;
			else
			...
			if (mod.metadata != null) {
				if (!string.IsNullOrEmpty(mod.metadata.links.nexusLink)) {
					...
					nexusLink.onClick.AddListener(
							() => OpenModLink(mod.metadata.links.nexusLink));
				}
				...
				if (!string.IsNullOrEmpty(mod.metadata.links.githubLink)) {
					...
					ghLink.onClick.AddListener(
							() => OpenModLink(mod.metadata.links.githubLink));
				}
		...
		public void ModKeybindsShow(Mod selected) {
			...
			ModKeyBinds.transform.GetChild(0)
					.GetChild(6)
					.GetComponent<Button>()
		...
		void CreateList() {
			if ((bool)ModSettings_menu.showCoreModsDf.GetValue() &&
					!coreModCheckbox.isOn) {
				coreModCheckbox.isOn = true;
				return;
			}
			Mod[] coreMods =
					ModLoader.LoadedMods.Where(x => x.ID.StartsWith("MSCLoader_")).ToArray();
			Mod[] modList =
					ModLoader.LoadedMods.Where(x => !x.ID.StartsWith("MSCLoader_")).ToArray();
			...
			if (coreModCheckbox.isOn) {
				ModListHeader("Core Mods", new Color32(101, 34, 18, 255),
						new Color32(254, 254, 0, 255));
				for (int i = 0; i < coreMods.Length; i++) {
					ModButton(coreMods[i].Name, coreMods[i].Version, coreMods[i].Author,
							coreMods[i]);
				}
			}
			if (modList.Length == 0) {
				ModListHeader("No mods installed", Color.blue, Color.white);
				if (ModLoader.InvalidMods.Count == 0) return;
				ModListHeader("Invalid/Broken Mods", new Color32(101, 34, 18, 255),
						new Color32(254, 254, 0, 255));
				for (int i = 0; i < ModLoader.InvalidMods.Count; i++) {
					GameObject invMod = Instantiate(ms.ModButton_Invalid);
					invMod.transform.GetChild(0).GetComponent<Text>().text =
							ModLoader.InvalidMods[i];
					invMod.transform.SetParent(modView.transform, false);
				}
				return;
			}
			if (updateMods.Length > 0) {
				ModListHeader("Mods with updates", new Color32(57, 57, 57, 255),
						new Color32(0, 255, 255, 255));
				for (int i = 0; i < updateMods.Length; i++) {
					ModButton(updateMods[i].Name, updateMods[i].Version, updateMods[i].Author,
							updateMods[i]);
				}
			}
			if (ModLoader.GetCurrentScene() == CurrentScene.MainMenu) {
				if (menuMods.Length > 0) {
					ModListHeader("Loaded Mods", new Color32(57, 57, 57, 255),
							new Color32(0, 255, 255, 255));
					for (int i = 0; i < menuMods.Length; i++) {
						if (!menuMods[i].isDisabled && !menuMods[i].hasUpdate)
							ModButton(menuMods[i].Name, menuMods[i].Version, menuMods[i].Author,
									menuMods[i]);
					}
				}
				ModListHeader("Ready to load", new Color32(57, 57, 57, 255),
						new Color32(0, 255, 255, 255));
				for (int i = 0; i < modList.Length; i++) {
					if (!modList[i].LoadInMenu && !modList[i].isDisabled &&
							!modList[i].hasUpdate)
						ModButton(
								modList[i].Name, modList[i].Version, modList[i].Author, modList[i]);
				}
			} else {
				ModListHeader("Loaded Mods", new Color32(57, 57, 57, 255),
						new Color32(0, 255, 255, 255));
			...
			if (disMods.Length > 0) {
				ModListHeader("Disabled mods", new Color32(101, 34, 18, 255),
						new Color32(254, 254, 0, 255));
				for (int i = 0; i < disMods.Length; i++) {
					ModButton(
							disMods[i].Name, disMods[i].Version, disMods[i].Author, disMods[i]);
				}
			}
		...
		public void toggleVisibility() {
			if (!settingViewContainer.activeSelf) {
				noOfMods.text = string.Format(
						"<color=orange><b>{0}</b></color> Mods", ModLoader.LoadedMods.Count - 2);
				CreateList();
				page = 0;
				SetScrollRect();
