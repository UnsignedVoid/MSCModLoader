...
namespace MSCLoader {
	...
	public static class PlayMakerExtensions {
		...
		public static PlayMakerFSM GetPlayMaker(
				this GameObject obj, string playMakerName) {
			...
			if (pm != null) {
				...
				if (fsm != null) return fsm;
					...
					ModConsole.Error(string.Format(
							"GetPlayMaker: Cannot find <b>{0}</b> in GameObject <b>{1}</b>",
							playMakerName, obj.name));
					return null;
				}
			}
			ModConsole.Error(string.Format(
					"GetPlayMaker: Cannot find any Playmakers in GameObject <b>{0}</b>",
					obj.name));
			return null;
		}
		...
		public static FsmState GetPlayMakerState(this GameObject obj, string stateName) {
			...
			if (pm != null) {
				...
				ModConsole.Error(string.Format(
						"GetPlayMakerState: Cannot find <b>{0}</b> state in GameObject <b>{1}</b>",
						stateName, obj.name));
				return null;
			}
			ModConsole.Error(string.Format(
					"GetPlayMakerState: Cannot find any Playmakers in GameObject <b>{0}</b>",
					obj.name));
			return null;
		}
		/// <summary>
