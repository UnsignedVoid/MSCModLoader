...
#pragma warning disable CS1591
	public class ConsoleView : MonoBehaviour {
		...
		public GameObject viewContainer; // Container for console view, should be a child
																		 // of this GameObject
		...
		~ConsoleView() { controller.logChanged -= onLogChanged; }

		public void toggleVisibility() {
			if (viewContainer.activeSelf)
				viewContainer.transform.GetChild(5)
		...
		// Event that should be called by anything wanting to submit the current input to
		// the console.
		...
		private void Update() {
			if (inputField.isFocused) {
				...
				if (inputField.text.Length > 0 && Input.GetKey(KeyCode.RightArrow)) {
					...
					List<string> found =
							controller.commands.Keys.Where(w => w.StartsWith(inputField.text))
									.ToList();
					if (found.Count > 0) {
