...
namespace MSCLoader {
	...
	public class ModAudio : MonoBehaviour {
		...
		/// <param name="doStream">Stream from HDD instead of loading to memory
		/// (recommended)</param> <param name="background">Load file in
		/// background</param>
		public void LoadAudioFromFile(string path, bool doStream, bool background) {
			...
			try {
				...
				audioSource.clip =
						Manager.Load(stream, format, filename, doStream, background, true);
			} catch (Exception e) {
		...
		public TimeSpan Time() {
			if (audioSource.clip != null) return TimeSpan.FromSeconds(audioSource.time);
			else
