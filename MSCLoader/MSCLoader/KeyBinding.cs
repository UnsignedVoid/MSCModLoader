...
#pragma warning disable CS1591
	...
	public class KeyBinding : MonoBehaviour {
		...
		public void LoadBind() {
			...
			modifierButton.GetComponent<Button>().onClick.AddListener(
					() => ChangeKeyCode(true, true));
			keyButton.GetComponent<Button>().onClick.AddListener(
					() => ChangeKeyCode(true, false));
		...
		void Update() {
			if (reassignKey) {
				// Checks if key is pressed and if button has been pressed indicating wanting
				// to re-assign
				if (Input.anyKeyDown) {
					...
					for (int i = 0; i < keyCodes.Length; i++) {
						if (Input.GetKeyDown(keyCodes[i])) {
							if (keyCodes[i] != KeyCode.Mouse0 &&
									keyCodes[i] != KeyCode.Mouse1) // LMB = cancel
							{
		...
		public void ChangeKeyCode(bool toggle, bool modifier) {
			...
			ismodifier = modifier;
			if (toggle) {
				if (modifier) buttonModImage.color = toggleColor;
				else
					buttonImage.color = toggleColor;
			} else {
				if (modifier) buttonModImage.color = originalColor;
				else
