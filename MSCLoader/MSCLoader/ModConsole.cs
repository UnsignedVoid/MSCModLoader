...
#pragma warning disable CS1591
		...
		private Keybind consoleKey =
				new Keybind("Open", "Open console", KeyCode.BackQuote);

		public static Settings typing =
				new Settings("typeConsole", "Start typing when you open console", false);
		static Settings ConsoleFontSize =
				new Settings("consoleFont", "Change console font size:", 12, ChangeFontSize);
		...
		public override void ModSettingsLoaded() { ChangeFontSize(); }

		public static void ChangeFontSize() {
			console.logTextArea.fontSize =
					int.Parse(ConsoleFontSize.GetValue().ToString());
		}
		...
		public override void OnMenuLoad() {
			try {
				...
				ModUI.ShowMessage(
						$"Fatal error:{System.Environment.NewLine}<color=orange>{e.Message}</color>{System.Environment.NewLine}Please install modloader correctly.",
						"Fatal Error");
			}
...
#pragma warning restore CS1591
		...
		public static void Print(string str) {
			...
			System.Console.WriteLine(string.Format(
					"MSCLoader Message: {0}", Regex.Replace(str, "<.*?>", string.Empty)));
		}
		/// <summary>
		...
		public static void Error(string str) {
			try {
				...
				console.controller.AppendLogLine(
						string.Format("<color=red><b>Error: </b>{0}</color>", str));
			} catch {}
			System.Console.WriteLine(string.Format(
					"MSCLoader ERROR: {0}", Regex.Replace(str, "<.*?>", string.Empty)));
		}
		...
		public static void Warning(string str) {
			try {
				console.setVisibility(true);
				console.controller.AppendLogLine(
						string.Format("<color=yellow><b>Warning: </b>{0}</color>", str));
			} catch {}
			System.Console.WriteLine(string.Format(
					"MSCLoader WARNING: {0}", Regex.Replace(str, "<.*?>", string.Empty)));
		}
	}
}