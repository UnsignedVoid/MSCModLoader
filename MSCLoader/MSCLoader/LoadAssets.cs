...
namespace MSCLoader {
	...
	public static class LoadAssets {
		/// <summary>
		/// Make GameObject Pickable, make sure your GameObject has Rigidbody and
		/// colliders attached.
		/// </summary>
		...
		/// </summary>
		/// <example>
		/// You need to enter file name from your mod's asset folder.<code
		/// source="Examples.cs" region="LoadTexture" lang="C#" title="Example for change
		/// texture when we press key" /></example> <param name="mod">Mod
		/// instance.</param> <param name="fileName">File name to load from assets folder
		/// (for example "texture.dds")</param> <param name="normalMap">Normal mapping
		/// (default false)</param> <returns>Returns unity Texture2D</returns>
		public static Texture2D LoadTexture(
				Mod mod, string fileName, bool normalMap = false) {
			string fn = Path.Combine(ModLoader.GetModAssetsFolder(mod), fileName);

			if (!File.Exists(fn)) {
				throw new FileNotFoundException(
						string.Format("<b>LoadTexture() Error:</b> File not found: {0}{1}", fn,
								Environment.NewLine),
						fn);
			}
			...
			if (ext == ".png" || ext == ".jpg") {
				...
				throw new NotSupportedException(string.Format(
						"<b>LoadTexture() Error:</b> Texture not supported: {0}{1}", fileName,
						Environment.NewLine));
			}
		}
		...
		/// <param name="fileName">File name to load from assets folder (for example
		/// "beer.obj")</param> <param name="collider">Apply mesh collider to
		/// object</param> <param name="rigidbody">Apply rigidbody to object to affect
		/// gravity (don't do it without collider)</param> <returns>Returns unity
		/// GameObject</returns> <example>Example Code <code source="Examples.cs"
		/// region="LoadOBJ" lang="C#" />
		/// </example>
		[Obsolete("LoadOBJ is deprecated, please use AssetBundles instead.", true)]
		public static GameObject LoadOBJ(
				Mod mod, string fileName, bool collider = true, bool rigidbody = false) {
			Mesh mesh = LoadOBJMesh(mod, fileName);
			if (mesh != null) {
				GameObject obj = new GameObject();
				obj.AddComponent<MeshFilter>().mesh = mesh;
				obj.AddComponent<MeshRenderer>();
				if (rigidbody) obj.AddComponent<Rigidbody>();
				if (collider) {
					if (rigidbody) obj.AddComponent<MeshCollider>().convex = true;
					else
		...
		/// <param name="mod">Mod instance.</param>
		/// <param name="fileName">File name to load from assets folder (for example
		/// "beer.obj")</param> <returns>Returns unity Mesh</returns> <example>Example
		/// Code <code source="Examples.cs" region="LoadOBJMesh" lang="C#" />
		/// </example>
		...
		static Texture2D LoadTGA(string fileName) {
			using (var imageFile = File.OpenRead(fileName)) { return LoadTGA(imageFile); }
		}

		// DDS loader by
		// https://gist.github.com/tomazsaraiva/8be91104fa0f4a52e0a7629eb1e59844#file-loaddds-cs
		static Texture2D LoadDDS(string ddsPath) {
			try {
				...
				if (ddsSizeCheck != 124)
					throw new Exception(
							"Invalid DDS DXTn texture. Unable to read"); // this header byte should
																													 // be 124 for DDS image
																													 // files
				...
				Buffer.BlockCopy(
						data, DDS_HEADER_SIZE, dxtBytes, 0, data.Length - DDS_HEADER_SIZE);
				...
				ModConsole.Error(string.Format(
						"<b>LoadTexture() Error:</b>{0}Error: Could not load DDS texture",
						Environment.NewLine, ex.Message));
				if (ModLoader.devMode) ModConsole.Error(ex.ToString());
			...
			using (BinaryReader r = new BinaryReader(TGAStream)) {
				...
				if (bitDepth == 32) {
					...
					throw new Exception(string.Format(
							"<b>LoadTexture() Error:</b> TGA texture is not 32 or 24 bit depth.{0}",
							Environment.NewLine));
				}
