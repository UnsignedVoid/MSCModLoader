...
#pragma warning disable CS1591
	public static class MSCLoader {
		...
		private static void OutputLogChecker() {
			try {
				...
				using (FileStream stream =
									 File.OpenRead(Path.Combine("", @"mysummercar_Data\mainData"))) {
				...
				if (enLog) {
					using (var stream =
										 new FileStream(Path.Combine("", @"mysummercar_Data\mainData"),
		...
		static void AssemblyWatcher(object sender, AssemblyLoadEventArgs args) {
			...
			if (args.LoadedAssembly.GetName().Name == "System") {
				AppDomain.CurrentDomain.AssemblyLoad -=
						AssemblyWatcher; // Remove from domain when done.
		...
		class InjectClickthroughFix {
			static bool Prefix() {
				if (UnityEngine.GUIUtility.hotControl != 0) { return false; }
		...
		static void InjectModLoader() {
			try {
				...
				HarmonyInstance.Create(nameof(MSCLoader))
