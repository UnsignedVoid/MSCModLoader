...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or
	...
	public partial class MP3Stream : IDisposable {
		...
		private void ReadFullyStream_StreamTitleChanged(
		...
		private static IMp3FrameDecompressor CreateFrameDecompressor(Mp3Frame frame) {
			WaveFormat waveFormat = new Mp3WaveFormat(frame.SampleRate,
					frame.ChannelMode == ChannelMode.Mono ? 1 : 2, frame.FrameLength,
					frame.BitRate);
			...
			if (playbackState != StreamingPlaybackState.Stopped) {
				if (bufferedWaveProvider != null) {
					...
					if (bufferedSeconds < 0.5 &&
							playbackState == StreamingPlaybackState.Playing && !fullyDownloaded) {
						Pause();
					} else if (bufferedSeconds > 3 &&
							playbackState == StreamingPlaybackState.Buffering) {
						Play();
		...
		private void OnPlaybackStopped(object sender, StoppedEventArgs e) {
			...
			if (e.Exception != null) {
				System.Console.WriteLine(
						string.Format("Playback Error {0}", e.Exception.Message));
			}
		}
	}
