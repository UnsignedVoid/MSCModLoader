...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or
															 // member
	public delegate void StreamTitleChangedEventHandler(
			object sender, StreamTitleChangedEventArgs e);
	public class StreamTitleChangedEventArgs : EventArgs {
		public readonly string Title;

		public StreamTitleChangedEventArgs(string title) { Title = title; }
	}
	public class ReadFullyStream : Stream {
		...
		private void ParseMetaInfo(byte[] metaInfo) {
			...
			string newStreamTitle =
					metaString.Split(new string[] { "';" }, StringSplitOptions.None) [0]
		...
		// This property is not currently supported and always thows a <see
		...
		// </summary>
		// <param name="buffer">An array of bytes to store the received data from the
		// ShoutcastStream.</param> <param name="offset">The location in the buffer to
		// begin storing the data to.</param> <param name="count">The number of bytes to
		// read from the ShoutcastStream.</param> <returns>The number of bytes read from
		// the ShoutcastStream.</returns>
		public override int Read(byte[] buffer, int offset, int count) {
			try {
				...
				bytesRead = 0;
				bytesLeftToMeta =
						((MetaInt - receivedBytes) > count) ? count : (MetaInt - receivedBytes);

				while (bytesLeftToMeta > 0 &&
						(read = netStream.Read(buffer, thisOffset, bytesLeftToMeta)) > 0) {
					leftToRead -= read;
					thisOffset += read;
					bytesRead += read;
					receivedBytes += read;
				...
				if (receivedBytes == MetaInt) {
					...
					if (metaLen > 0) {
						...
						int len = 0;
						while ((len += netStream.Read(metaInfo, len, metaInfo.Length - len)) <
								metaInfo.Length)
							;
				...
				while (leftToRead > 0 &&
						(read = netStream.Read(buffer, thisOffset, leftToRead)) > 0) {
					leftToRead -= read;
					thisOffset += read;
					bytesRead += read;
					receivedBytes += read;
		...
		// This Method is not currently supported and always throws a <see
		// cref="NotSupportedException"/>.
		// </summary>
		...
		public override long Seek(
				long offset, SeekOrigin origin) => throw new NotSupportedException();
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or
															 // member
