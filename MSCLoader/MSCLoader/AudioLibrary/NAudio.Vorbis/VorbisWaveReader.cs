...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or
	...
	public class VorbisWaveReader : WaveStream, ISampleProvider, IWaveProvider {
		...
		public override long Length => (long)(_reader.TotalTime.TotalSeconds *
				_waveFormat.SampleRate * _waveFormat.Channels * sizeof(float));

		public override long Position {
			get => (long)(_reader.DecodedTime.TotalSeconds * _reader.SampleRate *
					_reader.Channels * sizeof(float));
			set {
				if (value < 0 || value > Length)
					throw new ArgumentOutOfRangeException("value");

				_reader.DecodedTime = TimeSpan.FromSeconds(
						(double)value / _reader.SampleRate / _reader.Channels / sizeof(float));
			}
		}
		...
		public override int Read(byte[] buffer, int offset, int count) {
			...
			// get the buffer, creating a new one if none exists or the existing one is too
			// small
			var cb = _conversionBuffer ?? (_conversionBuffer = new float[count]);
			if (cb.Length < count) { cb = (_conversionBuffer = new float[count]); }

			// let Read(float[], int, int) do the actual reading; adjust count back to
			// bytes
		...
		public int Read(float[] buffer, int offset, int count) => _reader.ReadSamples(
				buffer, offset, count);
			...
			set {
				if (!_reader.SwitchStreams(value)) {
					throw new InvalidDataException(
							"The selected stream is not a valid Vorbis stream!");
				}
