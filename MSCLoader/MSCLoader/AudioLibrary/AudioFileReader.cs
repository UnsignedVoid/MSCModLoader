...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or
	...
	public enum AudioFormat { wav, mp3, aiff, ogg, flac, unknown = -1 }
	internal class AudioFileReader : WaveStream, ISampleProvider {
		...
		public override long Position {
			...
			set {
				...
				lock (obj) { readerStream.Position = DestToSource(value); }
			}
		}
		...
		public AudioFileReader(Stream stream, AudioFormat format) {
			...
			sourceBytesPerSample = (readerStream.WaveFormat.BitsPerSample / 8) *
		...
		private void CreateReaderStream(Stream stream, AudioFormat format) {
			...
			case AudioFormat.wav:
				...
				break;
			case AudioFormat.mp3: readerStream = new Mp3FileReader(stream); break;
			case AudioFormat.aiff: readerStream = new AiffFileReader(stream); break;
			case AudioFormat.ogg: readerStream = new VorbisWaveReader(stream); break;
			case AudioFormat.flac: readerStream = new FlacReader(stream); break;
			default:
		...
		public int Read(float[] buffer, int offset, int count) {
			object obj = lockObject;
			int result;
			lock (obj) { result = sampleChannel.Read(buffer, offset, count); }
			return result;
		}
