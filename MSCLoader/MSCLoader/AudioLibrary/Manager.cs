...
namespace AudioLibrary {
	public class Manager : MonoBehaviour {
		...
		public static AudioClip Load(string filePath, bool doStream = false,
				bool loadInBackground = true, bool useCache = true) {
			...
			audioClip = Load(streamReader.BaseStream, GetAudioFormat(filePath), filePath,
					doStream, loadInBackground, true);
		...
		public static AudioClip Load(Stream dataStream, AudioFormat audioFormat,
			...
			try {
				...
				AudioInstance audioInstance = new AudioInstance { reader = reader,
					samplesCount =
		...
		private static void EnsureInstanceExists() {
			if (!managerInstance) {
				managerInstance =
						new GameObject("Runtime AudioClip Loader Manger singleton instance");
				managerInstance.hideFlags = HideFlags.HideAndDontSave;
				managerInstance.AddComponent<Manager>();
			}
		}

		public static void SetAudioClipLoadState(
				AudioClip audioClip, AudioDataLoadState newLoadState) {
		...
		public static void SetAudioClipLoadType(
				AudioClip audioClip, AudioClipLoadType newLoadType) {
		...
		public static AudioFormat GetAudioFormat(string filePath) {
			...
			try {
				result = (AudioFormat)Enum.Parse(
						typeof(AudioFormat), GetExtension(filePath), true);
