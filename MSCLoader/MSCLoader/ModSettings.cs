...
#pragma warning disable CS1591
	public class ModSettings_menu : Mod {
		...
		private Keybind menuKey =
				new Keybind("Open", "Open menu", KeyCode.M, KeyCode.LeftControl);

		internal static Settings dm_disabler =
				new Settings("mscloader_dm_disabler", "Disable mods throwing errors", false);
		internal static Settings dm_logST = new Settings(
				"mscloader_dm_logST", "Log-all stack trace (not recommended)", false);
		internal static Settings dm_operr =
				new Settings("mscloader_dm_operr", "Log-all open console on error", false);
		internal static Settings dm_warn =
				new Settings("mscloader_dm_warn", "Log-all  open console on warning", false);
		internal static Settings dm_pcon =
				new Settings("mscloader_dm_pcon", "Persistent console", false);

		internal static Settings expWarning =
				new Settings("mscloader_expWarning", "Show experimental warning", true);
		internal static Settings modPath = new Settings(
				"mscloader_modPath", "Show mods folder", true, ModLoader.MainMenuPath);
		private static Settings modSetButton = new Settings("mscloader_modSetButton",
				"Show settings button in bottom right corner", true, ModSettingsToggle);
		internal static Settings forceMenuVsync =
				new Settings("mscloader_forceMenuVsync", "60FPS limit in Main Menu", true,
						VSyncSwitchCheckbox);
		internal static Settings openLinksOverlay = new Settings(
				"mscloader_openLinksOverlay", "Open URLs in steam overlay", true);
		internal static Settings showCoreModsDf = new Settings(
				"mscloader_showCoreModsDf", "Show core modules by default", true);
		internal static Settings skipGameIntro = new Settings(
				"mscloader_skipGameIntro", "Skip game Splash Screen", false, SkipIntroSet);
		internal static Settings syncLoad =
				new Settings("mscloader_syncLoad", "Load mods synchronously", false);

		private static Settings expUIScaling = new Settings("mscloader_expUIScaling",
				"Ultra-widescreen UI scaling", false, ExpUIScaling);
		private static Settings tuneScaling =
				new Settings("mscloader_tuneScale", "Tune scaling:", 1f, ChangeUIScaling);

		private static Settings checkLaunch =
				new Settings("mscloader_checkLaunch", "Every launch", true);
		private static Settings checkDaily =
				new Settings("mscloader_checkDaily", "Daily", false);
		private static Settings checkWeekly =
				new Settings("mscloader_checkWeekly", "Weekly", false);
		...
		public override void ModSettingsLoaded() {
			...
			if (!bool.TryParse(skipIntro, out introSkip)) {
				...
				ModConsole.Error(string.Format(
						"Excepted boolean, received '{0}'.", skipIntro ?? "<null>"));
			} else {
				skipGameIntro.Value = introSkip;
			}
			ModSettingsToggle();
			ExpUIScaling();
			if ((bool)checkLaunch.GetValue()) cfmu_set = 0;
			if ((bool)checkDaily.GetValue()) cfmu_set = 1;
			if ((bool)checkWeekly.GetValue()) cfmu_set = 7;
		}
		...
		private static void ChangeUIScaling() {
			if ((bool)expUIScaling.GetValue()) {
				ModUI.GetCanvas().GetComponent<CanvasScaler>().matchWidthOrHeight =
						float.Parse(tuneScaling.GetValue().ToString());
			}
		}
		private static void ExpUIScaling() {
			if ((bool)expUIScaling.GetValue()) {
				ModUI.GetCanvas().GetComponent<CanvasScaler>().screenMatchMode =
		...
		private static void SkipIntroSet() {
			...
			ini.Configuration.AssigmentSpacer = "";
			ini["MSCLoader"]["skipIntro"] =
					((bool)skipGameIntro.GetValue()).ToString().ToLower();
		...
		private static void VSyncSwitchCheckbox() {
			if (ModLoader.GetCurrentScene() == CurrentScene.MainMenu) {
				if ((bool)forceMenuVsync.GetValue()) QualitySettings.vSyncCount = 1;
				else
		...
		public override void OnMenuLoad() {
			try {
				...
				ModUI.ShowMessage(
						$"Fatal error:{Environment.NewLine}<color=orange>{e.Message}</color>{Environment.NewLine}Please install modloader correctly.",
						"Fatal Error");
			}
		}
		...
		public void CreateSettingsUI() {
			...
			if (!(bool)modSetButton.GetValue()) Button_ms.SetActive(false);
			ab.Unload(false);
		}
		...
		public static void SaveModBinds(Mod mod) {
			...
			for (int i = 0; i < binds.Length; i++) {
				if (binds[i].ID == null || binds[i].Vals != null) continue;
				Keybinds keybinds = new Keybinds { ID = binds[i].ID, Key = binds[i].Key,
		...
		public static void ResetSettings(Mod mod) {
			if (mod != null) {
				...
				for (int i = 0; i < set.Length; i++) {
					...
					if (original != null) { set[i].Value = original.Value; }
				}
		...
		public static void ResetSpecificSettings(Mod mod, Settings[] sets) {
			if (mod != null) {
				...
				for (int i = 0; i < sets.Length; i++) {
					...
					if (original != null) { sets[i].Value = original.Value; }
				}
		...
		public static void SaveSettings(Mod mod) {
			...
			for (int i = 0; i < set.Length; i++) {
				...
				Setting sets = new Setting { ID = set[i].ID, Value = set[i].Value };
		...
		public static void LoadSettings() {
			for (int i = 0; i < ModLoader.LoadedMods.Count; i++) {
				...
				string path =
						Path.Combine(ModLoader.GetModSettingsFolder(ModLoader.LoadedMods[i]),
								"settings.json");
				if (!File.Exists(path))
					SaveSettings(
							ModLoader.LoadedMods[i]); // create settings file if not exists.

				// Load and deserialize
				SettingsList settings =
						JsonConvert.DeserializeObject<SettingsList>(File.ReadAllText(path));
				ModLoader.LoadedMods[i].isDisabled = settings.isDisabled;
				try {
					if (ModLoader.LoadedMods[i].LoadInMenu &&
							!ModLoader.LoadedMods[i].isDisabled &&
							ModLoader.LoadedMods[i].fileName != null) {
					...
					ModConsole.Error(string.Format("Mod <b>{0}</b> throw an error!{1}",
							ModLoader.LoadedMods[i].ID, errorDetails));
					if (ModLoader.devMode) ModConsole.Error(e.ToString());
					System.Console.WriteLine(e);
				}
				if (Settings.Get(ModLoader.LoadedMods[i]).Count == 0) continue;

				for (int j = 0; j < settings.settings.Count; j++) {
					Settings set =
							Settings.modSettings.Find(x => x.Mod == ModLoader.LoadedMods[i] &&
											x.ID == settings.settings[j].ID);
		...
		public override void OnLoad() {
			GameObject.Find("Systems")
					.transform.Find("OptionsMenu")
					.gameObject.AddComponent<ModSettingButtonHandler>()
					.modSettingButton = Button_ms;
		...
		public class ModSettingButtonHandler : MonoBehaviour {
			...
			void OnEnable() {
				if ((bool)modSetButton.GetValue()) modSettingButton.SetActive(true);
			...
			void OnDisable() {
				if (modSettingButton != null) modSettingButton.SetActive(false);
			}
		}
	}
