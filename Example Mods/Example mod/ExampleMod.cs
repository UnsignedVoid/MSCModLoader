...
namespace ExampleMod {
	public class ExampleMod : Mod {
		...
		private Keybind testKey =
				new Keybind("KeyID", "Key name", KeyCode.L, KeyCode.LeftControl);

		// Called when the mod is loaded
		public override void OnLoad() {
			Keybind.Add(this, testKey); // register keybind for this class
			ConsoleCommand.Add(
					new ExampleCommand()); // Add command console from ExampleCommand class
		...
		public override void Update() {
			if (testKey.IsDown()) { ModConsole.Print("Key is pressed!"); }

			if (testKey.IsPressed()) { ModConsole.Print("Key is held!"); }
		}
	}
}
